...
use pasture_core::nalgebra::{Point, Point3};
use point_cloud_viewer::renderer::settings::{BaseRenderSettings, Grid, PointCloudRenderSettings, PointColor, PointShape, Color, PointSize};
...
fn main() {
    ...
    options.run(|render_thread| {
        ...
        // add point cloud
        let point_buffer = utils::small_example_point_cloud(Point3::new(0.0, 0.0, 0.0), 500);
        let point_cloud_id = window.add_point_cloud(&point_buffer).unwrap();

        // (optional) configure point cloud settings
        window.set_point_cloud_settings(point_cloud_id, PointCloudRenderSettings {
