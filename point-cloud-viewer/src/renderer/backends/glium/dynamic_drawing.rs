...
use glium::vertex::{VerticesSource, MultiVerticesSource};
use std::borrow::Cow;
    ...
    fn from(i: &'b ModifyVertexFormat<'b>) -> Self {
        match i.old_vertices_source {
            VerticesSource::VertexBuffer(buffer, _vertex_format, is_per_instance) =>
                VerticesSource::VertexBuffer(buffer, i.new_vertex_format.as_ref().unwrap(), is_per_instance),
