...
impl Display for RendererError {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), fmt::Error> {
        match self {
            ...
            RendererError::WindowClosed{ .. } =>
                write!(f, "The window is closed."),

            RendererError::Graphics{ source } =>
                write!(f, "Gpu error: {}", source),

            RendererError::UnsupportedOperation{ backend_name, operation_name, platform_specific: true } =>
                write!(f, "The graphics backend `{}` does not support `{}` on this platform.", *backend_name, *operation_name),

            RendererError::UnsupportedOperation{ backend_name, operation_name, platform_specific: false } =>
                write!(f, "The graphics backend `{}` does not support `{}`.", *backend_name, *operation_name),
            ...
            RendererError::AttributeMismatch { attribute, problem } => match problem {
                AttributeMismatchType::DoesNotExist => write!(f, "The attribute  {} is not present in the point cloud.", attribute),
                AttributeMismatchType::WrongType => write!(f, "The attribute  {} is of the wrong type.", attribute),
            }
        }
    }
