...
def make_y_insertion_rate(ax, test_runs, index):
    ys = [i[index]["insertion_rate"]["insertion_rate_points_per_sec"] for i in test_runs]
    ax.set_ylabel("Insertion rate | points/s")
    ax.set_ylim(bottom=0, top=max(ys) * 1.1)
    return ys


def draw_y_latency(ax, xs, test_runs, index, x_log=False):
    ax.set_ylabel("Latency | seconds")
    latencies = [i[index]["latency"] for i in test_runs]
    bxpstats = [
        {
        ...
        } if i is not None else None for i in latencies
    ]
...
def make_x_nr_threads(ax, test_runs):
    ...
    xs = [int(i["config"]["num_threads"]) for i in test_runs]
...
def make_x_cache_size(ax, test_runs):
    ...
    ax.set_xscale("log")
    return [int(i["config"]["max_cache_size"]) for i in test_runs]


def make_x_node_size(ax, test_runs):
    ax.set_xlabel("Max Node Size | nr points")
    ax.set_xscale("log")
    return [int(i["config"]["max_node_size"]) for i in test_runs]


def make_x_priority_function(ax, test_runs):
    labels = [i["config"]["task_priority_function"] for i in test_runs]
...
def plot_insertion_rate_by_nr_threads(test_runs, index):
    ...
    ys = make_y_insertion_rate(ax, test_runs, index)
    ax.scatter(xs, ys)
    ax.set_title(index)
    fig.savefig(f"{index}-insertion_rate_by_nr_threads.pdf", format="pdf", bbox_inches="tight")
...
def plot_latency_by_nr_threads(test_runs, index):
    ...
    ax.set_title(index)
    fig.savefig(f"{index}-latency_by_nr_threads.pdf", format="pdf", bbox_inches="tight")
...
def plot_insertion_rate_by_cache_size(test_runs, index):
    ...
    ax.scatter(xs, ys)
    ax.set_title(index)
    fig.savefig(f"{index}-insertion_rate_by_cache_size.pdf", format="pdf", bbox_inches="tight")
...
def plot_latency_by_cache_size(test_runs, index):
    ...
    ax.set_title(index)
    fig.savefig(f"{index}-latency_by_cache_size.pdf", format="pdf", bbox_inches="tight")
...
def plot_insertion_rate_by_node_size(test_runs):
    ...
    ys = make_y_insertion_rate(ax, test_runs, "sensor_pos_index")
    ax.scatter(xs, ys)
...
def plot_latency_by_node_size(test_runs):
    ...
    ax.set_title("sensor_pos_index")
    fig.savefig(f"sensor_pos_index-latency_by_node_size.pdf", format="pdf", bbox_inches="tight")
...
def plot_insertion_rate_by_priority_function(test_runs, title):
    ...
    ys = make_y_insertion_rate(ax, test_runs, "octree_index")
    ax.bar(xs, ys, 0.7)
    ax.set_title(title)
    fig.savefig(f"{title}-insertion_rate_by_priority_function.pdf", format="pdf", bbox_inches="tight")
...
def plot_latency_by_priority_function(test_runs, title):
    ...
    ax.set_title(title)
    fig.savefig(f"{title}-latency_by_priority_function.pdf", format="pdf", bbox_inches="tight")
...
def plot_compare_insertion_rate(test_run):
    ...
    fig.savefig(f"compare_insertion_rate.pdf", format="pdf", bbox_inches="tight")
...
def plot_compare_latency(test_run):
    ...
    draw_y_latency(ax, xs, test_runs, "index")
    plt.xticks(xs, indexes)
    fig.savefig(f"compare_latency.pdf", format="pdf", bbox_inches="tight")
...
def plot_compare_query_time(test_run):
    ...
    fig.savefig(f"compare_queryperf.pdf", format="pdf", bbox_inches="tight")
...
def plot_insertion_rates_by_disk_speed(data):
    ...
    ax.legend()
    fig.savefig(f"compare_insertion_rate_by_disk_speed.pdf", format="pdf", bbox_inches="tight")
...
def plot_latencies_by_disk_speed(data):
    ...
    ax.set_ylabel("Latency | seconds")
    ax.legend()
    fig.savefig(f"compare_latency_by_disk_speed.pdf", format="pdf", bbox_inches="tight")
...
def rename_tpf(run):
    ...
    tpf = run["config"]["task_priority_function"]
    if tpf in replacements:
        run["config"]["task_priority_function"] = replacements[tpf]
    return run


def tpf_hack(test_runs):
    """
    Renames the task priority functions to match the names that I chose in the thesis.
    For consistency.
    """
    renamed_runs = [rename_tpf(run) for run in test_runs]
    return [run for run in renamed_runs if run["config"]["task_priority_function"] is not None]
...
def main():
    ...
    plot_insertion_rates_by_disk_speed(disk_speed_data)
    plot_latencies_by_disk_speed(disk_speed_data)
