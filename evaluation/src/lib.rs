...
use lidarserv_common::geometry::position::{CoordinateSystemError, I32CoordinateSystem};
use lidarserv_common::geometry::{points::PointType, position::CoordinateSystem};
...
pub fn read_points(
        ...
        || settings.points_file.extension().unwrap() == "laz"
    {
        ...
        points = result
            ...
            .drain(..)
            .map(|las_point| {
                // transform position into new coordinate system
                let point_original = las_point.position();
                let point_global = result.coordinate_system.decode_position(point_original);
                let point_local = coordinate_system.encode_position(&point_global).unwrap();

                Point {
                    position: point_local,
